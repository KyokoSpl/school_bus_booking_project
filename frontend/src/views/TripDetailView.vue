...
<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-else-if="error" class="text-center py-12">
      <svg class="w-16 h-16 mx-auto text-red-400 dark:text-red-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </svg>
      <h3 class="text-xl font-semibold mb-2 dark:text-gray-100">{{ error }}</h3>
    ...
    <div v-else-if="trip" class="grid lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <!-- Title & Description -->
        <div class="card p-6">
          <h1 class="text-3xl md:text-4xl font-bold mb-4 dark:text-gray-100">{{ trip.titel }}</h1>
          <p class="text-gray-600 dark:text-gray-400 text-lg leading-relaxed">{{ trip.beschreibung }}</p>
        </div>
        ...
        <div v-if="trip.highlights && trip.highlights.length > 0" class="card p-6">
          ...
          <ul class="grid md:grid-cols-2 gap-3">
            <li v-for="(highlight, index) in trip.highlights" :key="index" class="flex items-start dark:text-gray-300">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          <div v-if="trip.inkludiert && trip.inkludiert.length > 0" class="card p-6">
            <h2 class="text-xl font-semibold mb-4 text-green-600 dark:text-green-400 flex items-center">
            ...
            <ul class="space-y-2">
              <li v-for="(item, index) in trip.inkludiert" :key="index" class="flex items-start dark:text-gray-300">
                <svg class="w-5 h-5 mr-2 text-green-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span>{{ item }}</span>
          ...
          <div v-if="trip.nicht_inkludiert && trip.nicht_inkludiert.length > 0" class="card p-6">
            ...
            <ul class="space-y-2">
              <li v-for="(item, index) in trip.nicht_inkludiert" :key="index" class="flex items-start dark:text-gray-300">
      ...
      <div class="lg:col-span-1">
        <div class="card p-6 sticky top-4">
          <!-- Price -->
          <div class="text-center mb-6 pb-6 border-b dark:border-gray-700">
            <p class="text-gray-500 dark:text-gray-400 text-sm">Preis pro Person</p>
            <p class="text-4xl font-bold text-primary-600 dark:text-primary-400">{{ formatPrice(trip.preis_pro_person) }}</p>
          </div>
          ...
          <div class="space-y-4 mb-6">
            <div class="flex items-start">
              ...
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Abfahrt</p>
                <p class="font-medium dark:text-gray-100">{{ formatDate(trip.abfahrt_datum) }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ formatTime(trip.abfahrt_zeit) }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start">
              ...
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Rückkehr</p>
                <p class="font-medium dark:text-gray-100">{{ formatDate(trip.rueckkehr_datum) }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ formatTime(trip.rueckkehr_zeit) }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start">
              ...
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Abfahrtsort</p>
                <p class="font-medium dark:text-gray-100">{{ trip.abfahrtsort }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start">
              ...
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Verfügbare Plätze</p>
                <p class="font-medium dark:text-gray-100">
                  <span :class="trip.verfuegbare_plaetze <= 5 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">
          ...
          <div v-else-if="trip.verfuegbare_plaetze === 0" class="text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-2">Diese Reise ist leider ausgebucht.</p>
          </div>
          ...
          <!-- Login hint -->
          <p v-if="isBookable && !authStore.isAuthenticated" class="text-center text-sm text-gray-500 dark:text-gray-400 mt-3">
            <router-link :to="{ name: 'login', query: { redirect: route.fullPath } }" class="text-primary-600 dark:text-primary-400 hover:underline">
              Anmelden
            </router-link>
            oder
