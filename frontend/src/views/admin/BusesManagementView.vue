...
<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <h2 class="text-2xl font-bold">Busse verwalten</h2>
    ...
    <!-- Grid -->
    <div v-else-if="buses.length === 0" class="card p-8 text-center text-gray-500">
    ...
    <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="bus in buses" :key="bus.id" class="card p-6">
        <div class="flex justify-between items-start mb-4">
          <div>
            <h3 class="text-lg font-semibold">{{ bus.bezeichnung }}</h3>
            <p class="text-gray-500 font-mono">{{ bus.kennzeichen }}</p>
          </div>
          <span
            class="badge"
            :class="{
        ...
        <div class="space-y-2 text-sm mb-4">
          <div class="flex justify-between">
            <span class="text-gray-500">Sitzplätze:</span>
            <span class="font-medium">{{ bus.sitzplaetze }}</span>
          </div>
          <div v-if="bus.baujahr" class="flex justify-between">
            <span class="text-gray-500">Baujahr:</span>
            <span class="font-medium">{{ bus.baujahr }}</span>
          </div>
        </div>

        <div v-if="bus.ausstattung && bus.ausstattung.length > 0" class="mb-4">
          <p class="text-sm text-gray-500 mb-2">Ausstattung:</p>
        ...
        <div v-if="bus.notizen" class="mb-4 text-sm text-gray-600">
          <p class="text-gray-500">Notizen:</p>
          <p>{{ bus.notizen }}</p>
        </div>
    ...
    <div v-if="editModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white rounded-xl max-w-md w-full p-6">
        <h3 class="text-xl font-bold mb-4">{{ editModal.isNew ? 'Neuer Bus' : 'Bus bearbeiten' }}</h3>
        ...
        <form @submit.prevent="handleSave" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Sitzplätze *</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ausstattung (kommagetrennt)</label>
    ...
    <div v-if="deleteModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white rounded-xl max-w-md w-full p-6">
        <h3 class="text-xl font-bold mb-4">Bus löschen</h3>
